...
func TestInspectStreamClassWith(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "extensions image returns empty stream",
			inspector: &mockImagesInspector{
				inspectData: map[string]*types.ImageInspectInfo{
					testImage: {Labels: map[string]string{
						"io.openshift.os.streamclass": "rhel-10",
						"io.openshift.os.extensions":  "true",
					}},
				},
			},
			wantClass: "rhel-10",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sc, err := InspectStreamClassWith(context.Background(), tt.inspector, testImage)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
